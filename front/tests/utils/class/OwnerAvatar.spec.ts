...
it('can prompt all players to subscribe to the owner.', async () => {
  ...
  expect(checkPlayerSubscribedToOwnerSpy).toHaveBeenCalledTimes(numberOfPlayer)
  expect(venueActivitySpy).toHaveBeenCalledTimes(numberOfPlayer)
})
...
it('can start connection', async () => {
  ...
  promptOwnerSubscriptionToPlayersSpy.mockImplementation((): Promise<void> => {
    return new Promise((resolve) => resolve())
  })
...
it('can announce quiz number', () => {
  ...
  expect(dataStreamMockMethod.writeAnnounceText).toHaveBeenCalledWith('1問目！')
})
